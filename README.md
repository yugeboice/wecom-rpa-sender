...
客服 Agent 的企业微信桌面消息发送执行器。通过桌面客户端 RPA 发送消息，支持 **Windows** 和 **macOS**。
...
# 2. 编辑配置
notepad .env                           # 修改参数
notepad config\user_mapping.json       # 添加 user_id -> 搜索关键字映射
...
- 企业微信桌面客户端已安装并登录
- 屏幕分辨率建议 1920x1080+
- **注意**: pyautogui 在 Windows 上无需额外权限配置

### macOS
- Python 3.11+
- 企业微信桌面客户端已安装并登录
- 需要在 系统偏好设置 → 安全性与隐私 → 辅助功能 中授权终端/Python
...
│   ├── repositories/       # 数据访问层
│   ├── rpa/                # RPA 执行器（pyautogui + pyperclip）
│   ├── scheduler/          # APScheduler 定时任务
│   └── services/           # 业务逻辑
├── tests/                  # 测试
├── scripts/                # 启动/初始化脚本（.sh + .bat）
├── screenshots/            # RPA 截图
└── data/                   # SQLite 数据库
```
...
| 功能 | Windows | macOS |
|------|---------|-------|
| 激活窗口 | `ctypes.windll.user32.FindWindowW` + `SetForegroundWindow` | `osascript` AppleScript |
| 中文输入 | `pyperclip.copy` + `Ctrl+V` | `pyperclip.copy` + `Cmd+V` |
| 搜索快捷键 | `Ctrl+F` | `Cmd+F` |
| 剪贴板 | `pyperclip`（自动使用 win32 API） | `pyperclip`（自动使用 pbcopy/pbpaste） |
